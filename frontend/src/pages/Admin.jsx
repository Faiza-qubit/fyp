...
const handleEdit = (shoe) => {
  if (!shoe || !shoe._id) return console.error("Invalid shoe for editing");

  setEditingShoe(shoe);
  setFormData({
    name: shoe.name || "",
    brand: shoe.brand || "",
    price: shoe.price?.toString() || "0",
              ...
              <tbody>
                <AnimatePresence>
                  {filteredShoes.map((shoe) => (
                    <motion.tr key={shoe._id} initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }} className="border-t border-white/5 hover:bg-white/5 transition-colors">
                      <td className="p-6">
                      ...
                      <td className="p-6">
                        <div className="flex gap-2">
                          ...
                          <Button size="sm" variant="ghost" className="text-red-400 hover:text-red-300" onClick={() => handleDelete(shoe._id)}><Trash2 className="w-4 h-4" /></Button>
                        </div>
                      </td>
                    </motion.tr>
                  ))}
